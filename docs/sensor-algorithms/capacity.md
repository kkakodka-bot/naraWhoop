...
The raw waveform path caches completed windows by owner/device/window/proof digest; changed proof, bytes, revision or algorithm dependencies must invalidate reuse. A cache hit cannot authorize stale data, another source or a changed qualification. Measure both cold and warm paths, late historical data, conflicting duplicates and object overlap. Report cache hit rate, bytes fetched/decoded per revision and deterministic day-read time separately. The optional raw/model executor must remain bounded and unable to delay scalar/HRV publication indefinitely.

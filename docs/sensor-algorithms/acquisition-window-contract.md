...
The reader selects at most 1,024 active contracts and 8 MiB of contract bytes using SQL metadata admission, newest first. Larger scopes return an explicit acquisition budget reason with the retained subset. Referenced raw metadata is limited to 2,048 manifests; the independent model discovery default remains 256. Each raw proof references at most 300 objects, bounded by its 300 mapped one-second records and 8 MiB each of compressed/uncompressed bytes. All referenced objects must be present before any download; transport shard boundaries do not change signal coverage or feature values.

...
The deterministic deployment script preserves previous v2 containers and configuration until the exact candidate demonstrates advancing hosted polls and publication when eligible debt exists. Actual database/REST destinations must match the intended configuration without exposing credential values. Migration `20260919020000_physiology_worker_heartbeats.sql` supplies process-attributed liveness: a fresh deployment UUID, a distinct random UUID per JVM boot, and the immutable image source revision. Another process's global heartbeat cannot satisfy acceptance; a watchdog restart gets a new process identity rather than inheriting old progress. The global heartbeat remains for compatibility, not release attribution. Its timestamp follows the upstream temperature-dependency migration; no two migrations share a version.

...
Linux packaging preflight subsequently built and verified all 33 pinned CPU wheels, installed them offline with no broken requirements, and ran the actual checkpoint twice successfully under Linuxamd64 emulation. Repeatability is within each environment; cross-platform byte identity is not claimed. The first combined JVM image build was cancelled when the shared local Docker VM became unresponsive with 96% disk utilization. After SSH was restored, the combined image built successfully on the native VPS; its initial non-root/read-only-root probe exposed a Numba cache defect, repaired with private per-job cache directories in both production execution and the probe. See [preflight and native continuation evidence](linux-model-packaging-preflight.md).

The [VPS report](vps-resource-report.md) binds native synthetic measurements to their image/source/runtime/checkpoint and prospectively frozen diagnostic budgets. Access is no longer a blocker. Representative acquisition-qualified inputs, sustained whole-pipeline load, historical backfill cardinality and ingestion/database headroom remain required before deployment approval; a synthetic resource-budget pass cannot authorize activation.
...
| wav2sleep four-stage challenger | Real released checkpoint execution on macOS and Linuxamd64 CPU, deterministic repeat, gap rejection | Offline package and native combined image checked; exact-image synthetic resource evidence separate | No device acquisition or PSG qualification | Shadow-only; not canonical/deployed |

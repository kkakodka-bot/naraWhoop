...
PR21 advanced during implementation. The integration includes its later heads `28a6b32e0140507dc75ece23286db4c814bbd5bc` and `a39a0d46e3ce74fa8c5aee646e3cbb481f6525a3`; the original starting SHA above is unchanged. Final whitespace and PR comparison use the latter actual target-branch base, not `main`. Its queue fairness, archive batching and dependency-invalidation repairs are retained alongside this candidate's deadlines, publication gates and isolated model execution.
...
Infrastructure tests also require PyYAML and local PostgreSQL/Compose tooling. `PHYSIOLOGY_INFRA_PYTHON` can select that separately recorded environment without modifying the pinned inference environment. The continuation used Python 3.10.18/PyYAML 6.0.2 for those tests; this is tooling evidence, not a qualified model environment.

...
## Root independent review: process-scoped counter export

Reviewed the later Apple/Android PhoneComputeRuntime and existing TestBundleAssembler changes directly. The export contains fixed aggregate counter keys, random non-owner runtime generation, wall/monotonic observation bounds, build/source markers and explicit unobserved prior-process coverage. It is included through the existing redaction, size cap and user report-review flow. No background execution, physiological calculation, credential access, identity mutation, counter reset or raw health-value export is added by report creation. Final guards still reject numerical entry; Android now makes admission/count snapshots coherent under one lock. Apple test resets are reported and remain test-process-only.

Meaningful reported executions are separate artifacts: Swift app-host36 and Android32, zero failures/skips for the new export/report path. The root reviewed the code; final frozen-source execution will independently rerun the broader suites. Current-process snapshots cannot establish an unobserved crash/restart tail, widget/watch process or an elapsed soak; those remain explicit physical evidence requirements.

Root also independently reviewed the raw-family regression repair: deterministic day reading uses available manifests while each raw feature retains per-window completeness, and model assembly still requires the exact complete object set. Only explicit catalogue identity/budget failures become raw-lane abstentions; SQL/cancellation/unexpected exceptions continue to fail for retry. The new regression passes real scalar inputs through reader, scorer, publication and API while the raw family emits its failure reason and performs no object download. No reference approval or numerical formula was added.

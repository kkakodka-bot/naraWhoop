...
object ServerScoreDisplay {
    fun hrvRmssd(context: Context, day: String, overlay: ServerScoreDayCache?): Double? {
        if (!ServerScoringSettings.isEnabled(context) || overlay?.day != day || overlay.features["hrv"]?.isCanonicalAvailable != true) return null
    ...
    fun sleepTotalMin(context: Context, day: String, overlay: ServerScoreDayCache?): Double? {
        if (!ServerScoringSettings.isEnabled(context) || overlay?.day != day || overlay.features["sleep"]?.isCanonicalAvailable != true) return null

    ...
    suspend fun finish(sessionId: String, inputs: Inputs): WorkoutRow {
        val payload = session.prepareFinalization(sessionId, nowMs(), admit = { candidate ->
            ...
            val row = WorkoutRow(deviceId = frozen.deviceId, startTs = frozen.startMs / 1000, endTs = endMs / 1000,
                ...
                avgHr = samples.takeIf { !com.noop.analytics.PhoneComputeRuntime.finalHosted && it.isNotEmpty() }?.let { it.sumOf { hr -> hr.bpm } / it.size },
                maxHr = if (com.noop.analytics.PhoneComputeRuntime.finalHosted) null else samples.maxOfOrNull { it.bpm },
                ...
                energyKcal = if (!com.noop.analytics.PhoneComputeRuntime.finalHosted && samples.size >= 2) Calories.estimateBoutCalories(samples, inputs.profile,

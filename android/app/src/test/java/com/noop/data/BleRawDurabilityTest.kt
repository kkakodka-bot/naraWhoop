...
class BleRawDurabilityTest {
    ...
    @Test fun durableBleReceiptsAndContinuousImuArchivesShareRawLaneWithoutDroppingEither() = runBlocking {
        val raw = capture()
        db.withTransaction { raw.insert(db.openHelper.writableDatabase) }
        val archiveRecord = PushRawBatchRecord(
            rowId = 1,
            batchId = "imf1.fixture",
            capturedAt = 1_790_000_100,
            deviceClockRef = 1_790_000_100,
            wallClockRef = 1_790_000_100,
            startTs = 1_790_000_100,
            endTs = 1_790_000_101,
            frameCount = 1,
            byteSize = 3,
            framesBlob = byteArrayOf(4, 5, 6),
        )
        var archiveAcknowledged = false
        val archives = object : ImuExactArchiveSource {
            override fun pushDeviceIds() = setOf(owner.deviceId)
            override fun pushRecords(deviceId: String, afterTs: Long, limit: Int) = emptyList<ImuPushRecord>()
            override fun archiveRows(deviceId: String, limit: Int) =
                if (deviceId == owner.deviceId && !archiveAcknowledged) listOf(archiveRecord) else emptyList()
            override fun acknowledgeArchive(deviceId: String, row: PushRawBatchRecord) {
                assertEquals(owner.deviceId, deviceId)
                assertEquals(archiveRecord, row)
                archiveAcknowledged = true
            }
        }
        val dao = db.pushDao(archives, source)
        val captured = dao.binaryRows(PushBinaryTable.RAW_BATCH, owner.deviceId, 0, 1)
        assertEquals(raw.row.batchId, (captured.single() as PushBinaryRow.RawBatch).record.batchId)
        dao.acknowledgeBinary(PushBinaryTable.RAW_BATCH, owner.deviceId, captured)
        val archived = dao.binaryRows(PushBinaryTable.RAW_BATCH, owner.deviceId, 0, 1)
        assertEquals(archiveRecord, (archived.single() as PushBinaryRow.RawBatch).record)
        dao.acknowledgeBinary(PushBinaryTable.RAW_BATCH, owner.deviceId, archived)
        assertTrue(archiveAcknowledged)

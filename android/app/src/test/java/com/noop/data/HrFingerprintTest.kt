...
class HrFingerprintTest {
    ...
    @Test fun analysisFingerprintSqlRunsAndMovesForGravityOnlyCommit() {
        DriverManager.getConnection("jdbc:sqlite::memory:").use { db ->
            ...
            assertEquals("v4|h0:0|p0|r0|x0|g0|s0|e0|o0|t0|z0|q0:0|w50|w70|tagged0|registry", fingerprint(db))
            ...
            assertEquals("v4|h1:1000|p0|r2|x0|g1|s0|e0|o0|t0|z0|q0:0|w50|w70|tagged0|registry", fingerprint(db))
            db.createStatement().use { it.execute("INSERT INTO rrPacketProvenance DEFAULT VALUES") }
            assertEquals("v4|h1:1000|p0|r2|x0|g1|s0|e0|o0|t0|z0|q1:1|w50|w70|tagged0|registry", fingerprint(db))
        }
    }
    ...
    private fun createFingerprintTables(db: Connection) {
        db.createStatement().use { sql ->
            ...
            sql.execute("CREATE TABLE rrInterval(srcChannel INTEGER, tsSuspect INTEGER)")
            sql.execute("CREATE TABLE rrPacketProvenance(deviceId TEXT, ts INTEGER)")
            sql.execute("CREATE TABLE pairedDevice(id TEXT, brand TEXT, model TEXT, status TEXT)")
            listOf(
                "ppgHrSample", "respSample", "gravitySample", "sleepStateSample",

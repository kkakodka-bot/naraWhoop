...
class HrvPacketAdapterTest {
    @Test fun nativePacketAdapterRetainsZeroAndDoesNotBorrowOtherTransport() {
        ...
        assertEquals(5,observations.size); assertEquals(listOf(0,1,2),observations.take(3).map { it.ordinal })
        assertFalse(observations[1].originalAccepted)
        assertTrue(observations.all { it.userId == "u" && it.verifiedSpan == null && it.deviceFirmware == null })
        assertEquals("channel:7", observations.last().source)

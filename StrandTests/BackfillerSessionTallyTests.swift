...
final class BackfillerSessionTallyTests: XCTestCase {
    ...
    func testPhaseTimingSummaryLineNamesPhases() {
        ...
        XCTAssertTrue(line!.contains("complete local persistence path, including applicable IMU flush and raw preparation"), line ?? "")
        XCTAssertFalse(line!.contains("durable-store n="), "Legacy samples must not invent successful SQLite measurements")

...
| wav2sleep | Released pinned 9,666,562-byte checkpoint/config downloaded, hashed and executed twice on macOS CPU and twice in a network-disabled Linuxamd64 CPU environment; exact repeat within each environment; gaps reject | No wrist acquisition qualification, domain-transfer accuracy, held-out calibration, reviewed deployment rights, completed combined JVM image or actual VPS benchmark |
...
Bundle checks are integrity checks, not signatures or self-issued rights/activation approvals. After installation, capture and independently review the actual environment using the worker interpreter, then supply a qualified input receipt and operator activation. A Linuxamd64 CPU wheel bundle was built, hash-verified, installed offline, dependency-checked and used for actual released-checkpoint inference. The combined JVM image build was cancelled during emulated Kotlin compilation because of local Docker resource pressure; it is incomplete, not a compiler failure. No actual VPS resource results are supplied. See [packaging preflight](../../docs/physiology-v2/linux-model-packaging-preflight.md) and [model/input availability](../../docs/physiology-v2/learned-model-production-candidate.md).

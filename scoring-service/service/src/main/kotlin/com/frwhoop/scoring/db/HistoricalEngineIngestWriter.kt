...
import com.frwhoop.scoring.scoring.HistoricalScoreBundle
...
class HistoricalEngineIngestWriter(private val queue: HistoricalScoringWorkQueue) {
    fun write(item: HistoricalScoringWorkQueue.WorkItem, bundle: HistoricalScoreBundle, durationMs: Long): Long? {
    ...
    companion object {
        fun buildSnapshot(bundle: HistoricalScoreBundle): JSONObject {
            ...
            return JSONObject()
                .put("publicationMode", "shadow")
                .put("canonicalOutputsAllowed", false)
                .put("capabilityStates", JSONObject()
                    .put("hrv", JSONObject().put("state", if (bundle.result.daily.avgHrv == null) "unavailable" else "shadow")
                        .put("reason", if (bundle.rrIntervals > 0 && bundle.result.daily.avgHrv == null) "verified_beat_timing_unavailable" else "unqualified_history_algorithm"))
                    .put("spo2", JSONObject().put("state", if (bundle.result.daily.spo2Pct == null) "unavailable" else "imported")
                        .put("reason", if (bundle.result.daily.spo2Pct == null) "calibrated_source_unavailable" else "source_specific_import")))
        ...
        fun buildPayload(bundle: HistoricalScoreBundle): JSONObject {
        ...
        private fun sessionToJson(s: DetectedSleep,b: HistoricalScoreBundle,nap: Boolean): JSONObject {
            val stages = JSONArray()
            val observed = s.stages.filter(SleepStageSemantics::isKnownState)
            observed.forEach { stages.put(JSONObject().put("start",it.start).put("end",it.end)
                .put("stage",if (SleepStageSemantics.isSleep(it) && it.stage == "unknown") "sleep_unstaged" else it.stage)) }
            fun minutes(stage: String) = observed.filter { it.stage==stage }.sumOf { (it.end-it.start).coerceAtLeast(0) }/60.0
            val inBed = (s.end-s.start)/60.0
            val hasStages = observed.isNotEmpty()
            val asleep = if (hasStages) observed.filter(SleepStageSemantics::isSleep).sumOf { it.end-it.start }/60.0 else null

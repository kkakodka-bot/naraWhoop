...
class HistoricalSignalSampleReader(private val db: PostgresClient,
                         private val auxiliaryObjects: AuxiliaryObjectReader = AuxiliaryObjectReader(null)) {
    ...
    fun loadDay(userId: UUID, day: String, deviceId: UUID): DayInputs? = loadDay(userId,day,deviceId,false)
    fun loadHistoricalDay(userId: UUID, day: String, deviceId: UUID): DayInputs? = loadDay(userId,day,deviceId,true)
    private fun loadDay(userId: UUID, day: String, deviceId: UUID, historical: Boolean): DayInputs? =
        db.withConnection { conn ->
            ...
            val dayBounds = UserDayBounds.forDay(day, profileRow.zoneId)
            val bounds = dayBounds.copy(nightLo = dayBounds.dayLo - 30L * 3600L)

...
private actor MutableJournalFixture: PushSnapshotSource, PushTransport, PushProgressStore {
    ...
    func post(_ batch: PushBatch) throws -> PushTransportResponse {
        ...
        let receipt: [String: Any] = ["version": 1, "state": "verified_indexed", "receiptId": "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
            ...
            "schemaVersion": 1,
            "objectKey": "v3/research/users/\(owner.userID)/devices/\(PushDurabilityReceipt.canonicalDevice(owner: owner.userID, device: batch.deviceId))/\(batch.table.wireName)/synthetic/verified",
            "contentSha256": invalid ? String(repeating: "0", count: 64) : PushDurabilityReceipt.sha256(batch.body),

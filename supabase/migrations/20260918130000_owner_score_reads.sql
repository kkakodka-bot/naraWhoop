-- Owner fleet read path: phones authenticate with the same JWT or ingest token as push,
-- then read server_scoring_for_day through the Edge scores function (service_role).
-- Physiology-2 is the published read model for this hosted fleet. Multi-device accounts
-- select the most recently seen strap when the owner has not pinned a source.

begin;

-- Allow physiology-2 to be selected for readback without claiming ECG/PSG qualification.
do $$
declare
  r record;
begin
  for r in
    ...
    where c.conrelid = 'public.physiology_feature_qualifications'::regclass
      and c.contype = 'c'
  loop
    if r.def ilike '%qualification%'
       and (
         r.def ilike '%shadow%'
         or r.def ilike '%frwhoop-server-1%'
       )
    then
      execute format('alter table public.physiology_feature_qualifications drop constraint %I', r.conname);
    end if;
  end loop;
end $$;
...
alter table public.physiology_feature_qualifications
  add constraint physiology_feature_qualifications_qualification_check
    check (qualification in ('baseline','shadow','reference_qualified','published'));

alter table public.physiology_feature_qualifications
  drop constraint if exists physiology_feature_qualifications_check;
alter table public.physiology_feature_qualifications
  drop constraint if exists physiology_feature_qualifications_baseline_version;
alter table public.physiology_feature_qualifications
  add constraint physiology_feature_qualifications_baseline_version
    check (qualification <> 'baseline' or algorithm_version = 'frwhoop-server-1');
alter table public.physiology_feature_qualifications
  drop constraint if exists physiology_feature_qualifications_published_version;
alter table public.physiology_feature_qualifications
  add constraint physiology_feature_qualifications_published_version
    check (qualification <> 'published' or algorithm_version = 'frwhoop-physiology-2');

update public.physiology_feature_qualifications
  set qualification = 'published'
  where algorithm_version = 'frwhoop-physiology-2'
    and qualification = 'shadow';

update public.physiology_feature_defaults
  set algorithm_version = 'frwhoop-physiology-2', updated_at = now();
...
create or replace function public.get_owner_day_snapshot(p_user uuid, p_day date)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  return jsonb_build_object(
    'day', p_day,
    'server_scoring', public.server_scoring_for_day(p_user, p_day)
  );
end;
$$;
...
revoke all on function public.get_owner_day_snapshot(uuid, date) from public, anon, authenticated;
grant execute on function public.get_owner_day_snapshot(uuid, date) to service_role;
...
begin
  for f in
    ...
    where n.nspname = 'public'
      and p.proname in ('scoring_enqueue_day','physiology_enqueue_day','scoring_enqueue_legacy')
  loop

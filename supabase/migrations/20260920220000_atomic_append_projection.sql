-- One enrolled wire batch owns the input gate through every bounded projection statement.
-- Scoring cannot interrupt between statements; any failure rolls back the complete batch.
begin;
...
declare target_table text; conflict_columns text; columns_sql text; updates_sql text; part record;
begin
  ...
  -- Acquire before writing or taking the publication mutex. Never wait while holding a
  -- mutex needed by the scorer's separate HTTP publication connection.
  ...
  for part in select jsonb_agg(value order by ord) as rows
    ...
    group by (ord-1)/250 order by (ord-1)/250
  loop

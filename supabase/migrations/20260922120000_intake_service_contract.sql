...
-- Missing numerical producers are engineering debt, not unsupported hardware or a
-- claim that attaching better acquisition metadata alone would make them execute.
-- Keep genuine sensor/reference prerequisites in the producer matrix separately.
update public.compute_family_policy
set unavailable_status='unqualified', unavailable_reason='producer_not_implemented',
    policy_version='vps-only-producers-2'
where family in ('spot_hrv','live_workout','intraday_temperature','stress_events',
                 'biofeedback','live_coaching','insights');
-- Preserve previous immutable disposition rows while applying the current family policy.
-- Until the disposition worker fills this policy version, the current explicit policy
-- supplies missingness; a prior unsupported-hardware label must never override it.

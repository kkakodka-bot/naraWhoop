-- Expose five-minute HR summaries from the same immutable user/device/version result selected
-- for HRV and resting HR. Retain the inherited defaults/qualification policy and sample-backed
-- device choice; this migration does not qualify or promote an algorithm.
begin;

create function public.validate_heart_rate_window_ownership() returns trigger
language plpgsql set search_path='' as $$
declare windows jsonb; item jsonb;
begin
  windows := new.payload#>'{daily,heart_rate_windows}';
  if windows is null then return new; end if;
  if jsonb_typeof(windows) is distinct from 'array' then
    raise exception 'invalid heart rate window collection' using errcode='22023';
  end if;
  for item in select value from jsonb_array_elements(windows) loop
    if item->>'user_id' is distinct from new.user_id::text
      or item->>'device_id' is distinct from new.device_id::text
      or nullif(item->>'method_version','') is null
      or jsonb_typeof(item->'start') is distinct from 'number'
      or jsonb_typeof(item->'end') is distinct from 'number' then
      raise exception 'invalid heart rate window ownership or provenance' using errcode='22023';
    end if;
    if (item->>'end')::numeric-(item->>'start')::numeric <> 300 then
      raise exception 'invalid heart rate window duration' using errcode='22023';
    end if;
  end loop;
  return new;
end $$;
revoke all on function public.validate_heart_rate_window_ownership() from public,anon,authenticated;
create trigger validate_heart_rate_window_ownership before insert or update on public.server_physiology_results
  for each row execute function public.validate_heart_rate_window_ownership();
